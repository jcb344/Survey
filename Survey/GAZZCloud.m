...
-(NSString*)JSONForArray:(NSArray*)array{
    ...
    for (int i = 0; i<[array count]; i++) {
        ...
        for (int j = 0; j<[keys count]; j++) {
            if (j != ([keys count]-1) ) {
                if ([[[array objectAtIndex:i] objectForKey:[keys objectAtIndex:j]] isKindOfClass:[NSNumber class]]) {
                    outString = [outString stringByAppendingFormat:@" \"%@\":%f,",[keys objectAtIndex:j],[[[array objectAtIndex:i] objectForKey:[keys objectAtIndex:j]] doubleValue]];
                }
                else if ([[[array objectAtIndex:i] objectForKey:[keys objectAtIndex:j]] isKindOfClass:[NSString class]]){
                    outString = [outString stringByAppendingFormat:@" \"%@\":\"%@\",",[keys objectAtIndex:j],[[[array objectAtIndex:i] objectForKey:[keys objectAtIndex:j]] stringByReplacingOccurrencesOfString:@"\"" withString:@""] ];
                }
